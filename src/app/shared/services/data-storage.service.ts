...
export class DataStorageService 
{
	...
	storeData() {
		...
		this.http.put(`${environment.firebaseEndpoint}data.json`, { tags, books, collections })
			.subscribe(
				response => {
	...
	fetchData() {
		this.http.get<{ tags: Tag[], books: Book[], collections: Collection[] }>(`${environment.firebaseEndpoint}data.json`)
			.pipe(
				map(data => {
